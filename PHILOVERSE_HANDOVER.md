...
### 치명
1. **가로 오버플로**: 상단바(`#topbar`)에 로고·통계·검색창·버튼 4개가 한 줄로 들어가 390px 폭을 넘김. `#btns` 버튼들이 폭 31px로 찌그러져 글자가 세로로 줄바꿈되고, 레이아웃 뷰포트가 560px까지 늘어남 → 첫 진입 모달과 하단 카드의 오른쪽이 잘림
2. **의미 검색 패널이 항상 열린 채 시작** (데스크톱 포함): `#search-panel{display:flex}`(ID 선택자)가 `.side-panel{display:none}`을 덮어씀. 다른 패널을 열면 두 패널이 겹침

### 주요
3. 패널 버튼 6개가 두 줄로 줄바꿈되어 캔버스 상단을 가림. `#hint`("드래그·휠 줌")가 효과 버튼과 겹침. 휠이 없는 모바일에 맞지 않는 문구
4. 터치 타깃이 작음: 패널 버튼 높이 24px, 상단 버튼 11px 글자 (권장 44px)
5. 사이드 패널이 고정폭 240px(주제 항로 320px)이고 좌상단에 붙어 있음. 모바일에서는 하단 시트나 전체폭 패널이 적합
6. 사조 영향 그래프: 컨트롤 박스가 화면 상단 1/4을 차지하고 노드 라벨이 서로 겹침. 헤더 제목이 두 줄로 줄바꿈
7. 글자 크기: 본문 대부분 9~11px, 대비가 낮은 회색(`rgba(255,255,255,0.25)`)이 많음
